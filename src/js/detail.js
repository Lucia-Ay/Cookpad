...
fetchData(null, data => {
  ...
  const isSaved = window.localStorage.getItem(`CookPad${recipeId}`);
  ...
  $detailContainer.innerHTML = `
    ...
    <div class="detail-content">
        <div class="title-wrapper">
            ...
            <button 
            class="btn btn-secondary has-state has-icon ${isSaved ? 'saved' : 'removed'}" 
        ...
        <h2 class="title-medium ingredient-title">
            Ingredients
            <span class="label-medium">For ${servings} Servings</span>
        </h2>

        ${ingredientItems ? `<ul class="body-large ingredient-list">
        ${ingredientItems}</ul>` : ''}
    </div>
  `;
});
